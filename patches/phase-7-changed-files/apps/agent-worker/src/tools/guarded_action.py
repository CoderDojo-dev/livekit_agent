"""The one and only path from a sensitive tool to an action (cookbook section 8).
  ...
  1. assemble context from the verified session,
  2. Decision proposes a candidate + confidence (low -> escalate, never force),
  3. Policy issues the binding verdict (audited server-side),
  4. only AUTHORIZED reaches Execution — dispatched idempotently and audited (Phase 7).
...
async def execute_guarded_action(run_context: RunContext, action_type: str, payload: dict) -> dict:
    ...
    # AUTHORIZED -> execute idempotently (one key per session+action; safe to retry).
    idempotency_key = run_context.session.userdata.new_idempotency_key(action_type)
    return await get_execution_client().execute(
        idempotency_key, action_type, context["session_id"], payload
    )