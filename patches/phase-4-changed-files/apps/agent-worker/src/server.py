...
caller context, and starts the worker. No business logic, no decisions live here.
"""
...
async def _prefetch_user_data(language: str) -> SessionUserData:
    ...
    if msisdn:
        ...
        if snapshot is not None:
            ...
            logger.info("context prefetched: caller=%s vip=%s", snapshot.full_name, snapshot.is_vip)
        else:
            logger.info("no context snapshot for caller msisdn=%s", msisdn)
    return user_data
